...
本文依 Android 0.16.47 的程式與介面整理（2026-09-05）。說明目前可操作的設定流程；介面改版後，按鈕位置可能調整。
...
| 相機／麥克風回報 `setup_required` | 先依 `setupType` 確認缺少權限或媒體前景服務。請使用者開啟 PickPico 完成授權或刷新，再查詢能力；不要讓 Agent 停止或重啟 Node。 |
