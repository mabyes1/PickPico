...
Canonical reference Worker from Android `0.16.3` onward:

```text
https://relay.pickpico.workers.dev
```

When a device first migrates from either historical `https://relay.mcpocket.workers.dev` or the intermediate `https://pickpico-relay.mcpocket.workers.dev` base URL, PickPico rotates its persisted relay Node ID and relay secret before connecting to the canonical Worker. The old capability URL therefore stops identifying the migrated phone, while unrelated app settings and the local MCP bearer remain untouched.
