...
| 🧭 **混合能力入口** | 常用能力直接呼叫，其餘從短索引取得參數後執行 | 18 個常用能力直接提供，保留全部 60 個能力。 |
...
執行 `command_run`、建立 `task_create` 任務，或使用直接裝置工具時，`agent` 為必填字串，請填入呼叫模型實際已知的名稱與版本。首頁與 Pico UI 使用此欄位顯示當前模型；單次命令不必先建立任務，也不能省略型號。純能力探索及任務狀態查詢不要求先登記模型。

不知道實際型號時，必須明填 `unknown (model not exposed)`，不可捏造。此欄位是自報的顯示資訊，並非身分驗證。更新 App 後，舊客戶端需要重新取得 MCP 工具結構。

PickPico 提供 18 個常用能力的完整參數，加上 12 個管理／探索入口，共 30 個頂層工具。其餘 38 個能力列在 command_run 的短索引；以 capability_status 精確取得參數與可用狀態，規格未變時可沿用。只有不確定能力時才搜尋，建議用 1–3 個保留動作意圖的英文關鍵詞，例如 calendar list、clipboard write。

```text
「看看手機畫面」
       ↓
直接呼叫 ui_inspect（或需要圖片時 screen_capture）
       ↓
取得 observationId 與唯一目標
       ↓
執行一個畫面操作，再觀察
       ↓
依結果繼續工作
...
Software product definition, system architecture, Android / MCP / Relay engineering, agent workflow, integration, testing, README & technical documentation.

### Kate Li

PickPico Dock product definition, industrial design, physical interaction design, Dock specification & design documentation, brand identity, UI/UX visual design, demo visual design & production.
